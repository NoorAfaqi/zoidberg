...
## 2. Changes Across Commits

### `init: separate file for class implementation`
Introduced `preprocessing.py` as a standalone reusable module, extracting the transform logic out of the notebook so the Streamlit app and training pipelines share the exact same preprocessing code without duplication.

### `add preprocessing pipeline notebook and README`
Added `Preprocessing_Pipeline.ipynb`, which:
- Defines all transforms (CLAHE, noise reduction, grayscale-to-RGB, augmentation)
- Runs a convolutional **autoencoder per class** to identify and remove outlier images (top 5% reconstruction-error samples in train, top 3% in val/test)
- Sets up `DataLoader`s with `ImageFolder` for all three splits
- Includes a visual batch check to verify preprocessing output

Also added the `README.md` project reference.

### `RESNET50_Training`
Added `Resnet50_Training.ipynb`:
- Two-phase fine-tuning: freeze all → train head for 3 epochs → unfreeze `layer3` + `layer4` at `lr=5e-6`
- Class-weighted cross-entropy loss with `label_smoothing=0.1`
- Adam optimizer, `ReduceLROnPlateau` scheduler, early stopping at patience=4
- Gradient clipping at 2.0, mixed precision via `GradScaler`
- Exports `resnet50_pneumonia.pth` and `classes.json`

**Results (run on CPU):**

| Class | Precision | Recall | F1 |
|---|---|---|---|
| BACTERIAL_PNEUMONIA | 0.88 | 0.83 | 0.86 |
| NORMAL | 0.82 | 0.81 | 0.81 |
| VIRAL_PNEUMONIA | 0.71 | 0.79 | 0.75 |
| **Overall** | **—** | **0.81** | **0.8142** |

### `EfficientNet-B4 Trained`
Added `EfficientNet-B4_Training.ipynb` with the same training scaffold but:
- Unfreezes last 4 feature blocks (not named layers) at epoch 3, `lr=1e-5`
...
| Class | Precision | Recall | F1 |
|---|---|---|---|
| BACTERIAL_PNEUMONIA | 0.86 | 0.55 | 0.67 |
| NORMAL | 0.54 | 0.94 | 0.68 |
| VIRAL_PNEUMONIA | 0.72 | 0.29 | 0.42 |
| **Overall** | **—** | **0.64** | **0.6162** |

### `All models trained`
Added `ConvNeXt_Training.ipynb` with the same configuration as EfficientNet-B4 except for the model head (`model.classifier[2]`), run on CUDA.

**Results:**

| Class | Precision | Recall | F1 |
|---|---|---|---|
| BACTERIAL_PNEUMONIA | 0.76 | 0.79 | 0.77 |
| NORMAL | 0.75 | 0.65 | 0.70 |
| VIRAL_PNEUMONIA | 0.63 | 0.71 | 0.66 |
| **Overall** | **—** | **0.72** | **0.7183** |
...
## 4. Three Questions on the Dataset and Preprocessing
...
## 5. Why ResNet50 Outperforms EfficientNet-B4 and ConvNeXt Here
